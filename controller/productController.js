...
const stripe = new Stripe('sk_test_51PxV8MKaw2ar0QFZuKHrN5kZ7XidQp6i7xJA8uEENnhOfOHJRNHLJJ5wIh4ShC7P4np4lKBBa7ekV7t109RmwdmB00loWDYMMC');
...
export const createPaymentIntentController = async (req, res) => {
  try {
    ...
    const { amount } = req.body;  // Expect the total amount in cents
    if (!amount || amount <= 0) {
    ...
    res.status(200).json({
      clientSecret: paymentIntent.client_secret, // Send client secret to client-side
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
  ...
  try {
    ...
    if (paymentIntent.status === 'succeeded') {
      const order = new orderModel({
        products: cart.map(item => item._id), // Store only product IDs in order
